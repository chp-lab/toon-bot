...
class Hooking(Resource):
    beaconbot_id = "B0e42aac13b8d547ba303b00f8b225aa2"
    ...
    get_userprofile_api = "http://127.0.0.1:5008/api/v1/get_userprofile"
    get_userprofile_body = {}

    check_in_api = "http://127.0.0.1:5008/api/v1/check_in"
    check_in_body = {}

    check_out_api = "http://127.0.0.1:5008/api/v1/check_out"
    check_out_body = {}
    ...
    def check_daily(self, one_id, today):
        cmd = """SELECT * FROM timeattendance WHERE timeattendance.employee_code='%s' AND timeattendance.date='%s' """ % (
            one_id, today)
        database = Database()
    ...
    def get_area(self, major, minor):
        TAG = "get_are:"
        database = Database()
        cmd = """SELECT rooms.address FROM `rooms` WHERE rooms.minor=%s AND rooms.major=%s""" % (
            minor, major)
        res = database.getData(cmd)
    ...
    def send_quick_reply(self, one_id, msg, payload):
        ...
        result = requests.post(
            self.onechat_url1, json=req_body, headers=self.sendmessage_headers)
        return result
    ...
    def get_onechat_profile(self, auth):
        ...
        r = requests.post(self.onechat_uri + "/manage/api/v1/getprofile",
                          headers=self.sendmessage_headers, json=payload)
    ...
    def post(self):
            ...
            if(data["event"] == 'message'):
                ...
                if('data' in data['message']):
                    ...
                    if(recv_data == "my_rec"):
                        ...
                        rec = res[0]['result'][0]
                        reply_msg = "เวลาเข้างานของคุณคือ %s %s สถานที่ %s" % (
                            rec['check_in'], rec['date'], rec['address'])
        ...
        if('uuid' in data):
            ...
            tmp_msg = "event_stage:%s, proximity:%s" % (
                data['event_stage'], data['proximity'])
            ...
            if(not self.is_area_exist(major, minor)):
                self.send_msg(
                    one_id, "ไม่พบพื้นที่ในระบบ major:%s minor:%s" % (major, minor))
                return module.success()
            ...
            user_profile = requests.post(
                self.get_userprofile_api, json=self.get_userprofile_body, verify=False)
            ...
            if((event_stage == 'enter') or (event_stage == 'proximity_change')):
                ...
                if (self.is_entred(one_id) and (event_stage == 'enter')):
                    ...
                    greeting_msg = """ยินดีต้อนรับสู่ %s""" % (
                        building[0]['result'][0]['address'])
                ...
                chekcovid = requests.post(
                    self.covid_api, json=covid_body, verify=False)
                ...
                for covid_status in covid_filter:
                    if covid_status['status'] != None:
                        ...
                        insert_user = requests.post(
                            self.check_in_api, json=self.check_in_body, verify=False)
                        print("this is insert_user :" +
                              json.dumps(insert_user.json()))
                        ...
                        sendmessage = requests.post(
                            self.sendmessage_url, json=self.sendmessage_body, headers=self.sendmessage_headers, verify=False)
                ...
                checkout = requests.post(
                    self.check_out_api, json=self.check_out_body, verify=False)
